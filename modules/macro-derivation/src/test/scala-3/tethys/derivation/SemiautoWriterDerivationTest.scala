...
class SemiautoWriterDerivationTest extends AnyFlatSpec with Matchers {
  ...
  it should "generate proper writer from WriterDescription" in {
    ...
    implicit val dWriter: JsonWriter[D] =
      jsonWriter[D](WriterDerivationConfig.withFieldStyle(FieldStyle.UpperCase))
  ...
  it should "derive writer for simple enum" in {
    implicit val oneWriter: JsonObjectWriter[SimpleEnum.ONE.type] =
      jsonWriter[SimpleEnum.ONE.type]
    implicit val twoWriter: JsonObjectWriter[SimpleEnum.TWO.type] =
      jsonWriter[SimpleEnum.TWO.type]
    implicit val simpleEnumWriter: JsonWriter[SimpleEnum] =
      jsonWriter[SimpleEnum]
  ...
  it should "derive writer for parametrized enum" in {
    implicit val oneWriter: JsonObjectWriter[ParametrizedEnum.ONE.type] =
      jsonWriter[ParametrizedEnum.ONE.type]
    implicit val twoWriter: JsonObjectWriter[ParametrizedEnum.TWO.type] =
      jsonWriter[ParametrizedEnum.TWO.type]
    implicit val parametrizedEnumWriter: JsonWriter[ParametrizedEnum] =
      jsonWriter[ParametrizedEnum]
