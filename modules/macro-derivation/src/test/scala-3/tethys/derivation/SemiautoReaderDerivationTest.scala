...
class SemiautoReaderDerivationTest extends AnyFlatSpec with Matchers {
  ...
  it should "derive reader for recursive type" in {
    implicit val recursiveReader: JsonReader[RecursiveType] =
      jsonReader[RecursiveType]
    ...
    implicit val reader: JsonReader[SimpleType] = jsonReader[SimpleType] {
      describe {
        ReaderBuilder[SimpleType]
          .extract(_.i)
          .from(_.d)
          .and(Symbol("e").as[Double])((d, e) => (d + e).toInt)
      }
    }
    ...
    implicit val reader: JsonReader[SimpleTypeWithAny] =
      jsonReader[SimpleTypeWithAny] {
        ReaderBuilder[SimpleTypeWithAny]
          ...
          .extract(_.i)
          .from(_.d)
          .and(Symbol("e").as[Int])((d, e) => d.toInt + e)
          .extract(_.d)
  ...
  it should "derive reader for simple enum" in {
    implicit val oneReader: JsonReader[SimpleEnum.ONE.type] =
      jsonReader[SimpleEnum.ONE.type]
    implicit val twoReader: JsonReader[SimpleEnum.TWO.type] =
      jsonReader[SimpleEnum.TWO.type]
    implicit val simpleEnumReader: JsonReader[SimpleEnum] =
      jsonReader[SimpleEnum]
  ...
  it should "derive reader for parametrized enum" in {
    implicit val oneReader: JsonReader[ParametrizedEnum.ONE.type] =
      jsonReader[ParametrizedEnum.ONE.type]
    implicit val twoReader: JsonReader[ParametrizedEnum.TWO.type] =
      jsonReader[ParametrizedEnum.TWO.type]
    implicit val parametrizedEnumReader: JsonReader[ParametrizedEnum] =
      jsonReader[ParametrizedEnum]
