...
//  }
  ...
  it should "kek" in {
    sealed trait Integration[+t]
    object Integration {
      case object Nope extends Integration[Nothing]
      case class Url(url: String) extends Integration[Nothing]
      case class Context[T](url: String, ctx: T) extends Integration[T]
    }
    case class SimpleTypeWithEnum(i: Int, s: String, d: Double, color: Integration[Int])

    implicit val integrationNopeWriter: JsonObjectWriter[Integration.Nope.type] = jsonWriter[Integration.Nope.type]
    implicit val integrationUrlWriter: JsonObjectWriter[Integration.Url] = jsonWriter[Integration.Url]
    implicit val integrationCtxWriter: JsonObjectWriter[Integration.Context[Int]] = jsonWriter[Integration.Context[Int]]
    implicit val integrationWriter: JsonObjectWriter[Integration[Int]] = jsonWriter[Integration[Int]]
    implicit val simpleTypeWithEnumWriter: JsonWriter[SimpleTypeWithEnum] = jsonWriter[SimpleTypeWithEnum]

    SimpleTypeWithEnum(228, "someString", 14.88, Integration.Nope).asTokenList shouldBe obj()
  }
}
