...
object ReaderDescriptionMacro {

  def readerDescription[A: c.WeakTypeTag](c: blackbox.Context)(builder: c.Expr[ReaderBuilder[A]]): c.Expr[ReaderDescription[A]] = {
    new ReaderDescriptorMacroImpl[c.type](c).readerDescription[A](builder)
  }

  private class ReaderDescriptorMacroImpl[C <: blackbox.Context](val c: C) extends ReaderBuilderUtils {
    import c.universe._
