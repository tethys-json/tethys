...
trait SemiautoDerivation {
  ...
  inline def jsonWriter[T <: Product](
  ...
  inline def jsonReader[T <: Product](
...
private[this] object SemiautoDerivation {
  ...
  def jsonWriterWithConfig[T: Type](config: Expr[WriterDerivationConfig])(using
      Quotes
  ): Expr[JsonObjectWriter[T]] =
    new SemiautoDerivationMacro(quotes).jsonWriterWithConfig[T](config)

  def jsonWriterWithDescription[T: Type](
      description: Expr[WriterDescription[T]]
  )(using
      Quotes
  ): Expr[JsonObjectWriter[T]] =
    new SemiautoDerivationMacro(quotes)
  ...
  def describeWriter[T <: Product: Type](builder: Expr[WriterBuilder[T]])(using
      Quotes
  ...
  def jsonReaderWithConfig[T: Type](config: Expr[ReaderDerivationConfig])(using
      Quotes
  ): Expr[JsonReader[T]] =
    new SemiautoDerivationMacro(quotes).jsonReaderWithConfig[T](config)

  def jsonReaderWithDescription[T: Type](
      description: Expr[ReaderDescription[T]]
  )(using Quotes): Expr[JsonReader[T]] =
    new SemiautoDerivationMacro(quotes)
  ...
  def describeReader[T <: Product: Type](builder: Expr[ReaderBuilder[T]])(using
      Quotes
