...
trait ReaderBuilderCommons extends ReaderBuilderUtils {
  ...
  private def extractDescription[T <: Product: Type](
      expr: Expr[ReaderBuilder[T]]
  ): MacroReaderDescription = expr match {
    // ===== ROOT =====
    ...
    case '{ ($rest: ReaderBuilder[T]).fieldStyle($style) } =>
      ...
      description.copy(config = '{
        (${ description.config }: ReaderDerivationConfig).withFieldStyle($style)
      })
    ...
    case '{ ($rest: ReaderBuilder[T]).strict } =>
      val description = extractDescription(rest)
      description.copy(config = '{
        (${ description.config }: ReaderDerivationConfig).strict
      })
      ...
      other.asTerm match {
        ...
        case Apply(
              Select(
                Apply(
                  TypeApply(
                    Select(
                      Apply(
                        TypeApply(Select(rest, "extract"), _),
                        List(BuilderField(f))
                      ),
                      "from"
                    ),
          ...
          val fsFields = fs.map(fTerm =>
            BuilderField
              .unapply(fTerm)
              .fold(Field.unlift(fTerm))(Field.fromBuilderField)
          )
          description.copy(operations =
            description.operations :+ ReaderMacroOperation.ExtractFieldValue(
              f.name,
              fsFields,
              fun
            )
        ...
        case Apply(
              Select(
                Apply(
                  TypeApply(
                    Select(
                      Apply(
                        TypeApply(Select(rest, "extractReader"), _),
                        List(BuilderField(f))
                      ),
                      "from"
                    ),
          ...
          description.copy(operations =
            description.operations :+ ReaderMacroOperation.ExtractFieldReader(
              f.name,
              fsFields,
              fun
            )
