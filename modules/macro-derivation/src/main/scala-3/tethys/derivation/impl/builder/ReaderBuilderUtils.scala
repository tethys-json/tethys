...
trait ReaderBuilderUtils extends Reflection {
  ...
  case class MacroReaderDescription(
  ...
  object MacroReaderDescription {
    def unlift[T: Type](
        wd: Expr[ReaderDescription[T]]
    ): MacroReaderDescription = {
      ...
      withoutInlining match {
        case '{
              ReaderDescription.apply[T]($config, ${ Varargs(operations) })
            } =>
          MacroReaderDescription(
            config,
            operations.map(ReaderMacroOperation.unlift[T])
          )
      }
    }
  }
    ...
    def lift: Expr[ReaderDescription.BuilderOperation] = self match {
      case ReaderMacroOperation.ExtractFieldAs(field, tpe, as, fun) =>
        (tpe.asType, as.asType) match {
          case ('[tpe], '[as]) =>
            '{
              ReaderDescription.BuilderOperation.ExtractFieldAs[as, tpe](
                ${ Expr(field) },
                ${ fun.asExprOf[Function[as, tpe]] }
              )
            }
        }
  ...
  object ReaderMacroOperation {
    ...
    def unlift[T: Type](
        bo: Expr[ReaderDescription.BuilderOperation]
    ): ReaderMacroOperation =
      bo match {
        case '{
              ReaderDescription.BuilderOperation.ExtractFieldAs
                .apply[as, tpe]($field, $fun)
            } =>
          ReaderMacroOperation.ExtractFieldAs(
            field.valueOrAbort,
            TypeRepr.of[tpe],
            TypeRepr.of[as],
            fun.asTerm
        ...
        case '{
              ReaderDescription.BuilderOperation.ExtractFieldValue.apply(
                $field,
                ${ Varargs(from) },
                $fun
              )
            } =>
          ReaderMacroOperation.ExtractFieldValue(
            field.valueOrAbort,
            from.map(f => Field.unlift(f.asTerm)),
            fun.asTerm
          )
        case '{
              ReaderDescription.BuilderOperation.ExtractFieldReader.apply(
                $field,
                ${ Varargs(from) },
                $fun
              )
            } =>
          ReaderMacroOperation.ExtractFieldReader(
            field.valueOrAbort,
            from.map(f => Field.unlift(f.asTerm)),
            fun.asTerm
          )
  ...
  protected def evalReaderConfig(
      configExpr: Expr[ReaderDerivationConfig]
  ): (Option[FieldStyle], Boolean) = {
    def parseConfigExpr(
      ...
      confExpr match {
        ...
        case '{ ReaderDerivationConfig.apply(Some($fieldStyle), $isStrict) } =>
          (
            ...
            isStrictExpr.orElse(Some(isStrict))
          )
        ...
        case '{
              ($config: ReaderDerivationConfig).withFieldStyle($fieldStyle)
            } =>
          parseConfigExpr(
            config,
            fieldStyleExpr.orElse(Some(fieldStyle)),
    ...
    val fieldStyle: Option[FieldStyle] = fieldStyleExpr.map { fs =>
      fs.asTerm.underlying.asExprOf[ConfigFieldStyle] match {
        ...
        case _ =>
          report.errorAndAbort(
            "fieldStyle in reader config can not be computed in compile-time"
          )
      }
    }

    val isStrict = isStrictExpr
      .map(
        _.asTerm.underlying
          .asExprOf[Boolean]
          .value
          .getOrElse(
            report.errorAndAbort(
              "isStrict in reader config can not be computed in compile-time"
            )
          )
      )
      .getOrElse(false)
