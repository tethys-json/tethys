...
import tethys.derivation.builder.{
  ...
  WriterDescription,
  FieldStyle as ConfigFieldStyle
}
...
trait WriterBuilderUtils extends Reflection {
  ...
  case class MacroWriteDescription(
    ...
    def lift[T: Type]: Expr[WriterDescription[T]] =
      '{
        WriterDescription[T](
          $config,
          ${ Expr.ofSeq(operations.map(_.lift[T])) }
        )
      }
  }
  ...
  object MacroWriteDescription {
    ...
    def unlift[T: Type](
      ...
      withoutInlining match {
        case '{
              WriterDescription.apply[T]($config, ${ Varargs(operations) })
            } =>
          MacroWriteDescription(
            TypeRepr.of[T],
            config,
            operations.map(WriterMacroOperation.unlift[T])
          )
      }
    }
  }
  ...
  sealed trait WriterMacroOperation { self =>
    def lift[T: Type]: Expr[WriterDescription.BuilderOperation[T]] =
      self match {
        ...
        case WriterMacroOperation.UpdatePartial(
              tpe,
              field,
              name,
              fun,
  ...
  object WriterMacroOperation {
    ...
    case class Update(
        tpe: TypeRepr,
        field: String,
        name: Expr[Option[String]],
        fun: Term,
        from: TypeRepr,
        to: TypeRepr
    ) extends WriterMacroOperation
    case class UpdateFromRoot(
        tpe: TypeRepr,
        field: String,
        name: Expr[Option[String]],
        fun: Term,
    ...
    case class UpdatePartialFromRoot(
        tpe: TypeRepr,
        field: String,
        name: Expr[Option[String]],
        fun: Term,
    ...
    case class Add(tpe: TypeRepr, field: Expr[String], fun: Term, to: TypeRepr)
        extends WriterMacroOperation

    def unlift[T: Type](
        bo: Expr[WriterDescription.BuilderOperation[T]]
    ): WriterMacroOperation =
      bo match {
        case '{
              WriterDescription.BuilderOperation.Remove.apply[tpe]($field)
            } =>
          WriterMacroOperation.Remove(TypeRepr.of[tpe], field.valueOrAbort)

        case '{
              WriterDescription.BuilderOperation.Update
                .apply[tpe, from, to]($field, $name, $fun)
            } =>
        ...
        case '{
              WriterDescription.BuilderOperation.Add
                .apply[tpe, to]($field, $fun)
            } =>
          WriterMacroOperation.Add(
            TypeRepr.of[tpe],
            field,
            fun.asTerm,
            TypeRepr.of[to]
  ...
  protected def evalWriterConfig(
      configExpr: Expr[WriterDerivationConfig]
  ): (Option[FieldStyle], Option[String]) = {
    @tailrec
    def parseConfigExpr(
      ...
      confExpr match {
        ...
        case '{
              WriterDerivationConfig.apply(
                Some($fieldStyle),
                Some($discriminator)
              )
            } =>
          (
            fieldStyleExpr.orElse(Some(fieldStyle)),
            discriminatorExpr.orElse(Some(discriminator))
          )
        ...
        case '{
              ($config: WriterDerivationConfig).withFieldStyle($fieldStyle)
            } =>
          parseConfigExpr(
            config,
            fieldStyleExpr.orElse(Some(fieldStyle)),
        ...
        case '{
              ($config: WriterDerivationConfig).withDiscriminator(
                $discriminator
              )
            } =>
          parseConfigExpr(
        ...
        case _ =>
          report.errorAndAbort(
            s"Config parsing error. Unknown expr: ${confExpr.asTerm.show}"
          )
      }

    val (fieldStyleExpr, discriminatorExpr) =
      parseConfigExpr(configExpr, None, None)
    ...
    val fieldStyle: Option[FieldStyle] = fieldStyleExpr.map { fs =>
      fs.asTerm.underlying.asExprOf[ConfigFieldStyle] match {
        ...
        case '{ ConfigFieldStyle.CapitalizedKebabCase } =>
          FieldStyle.capitalizedKebabCase
        case '{ ConfigFieldStyle.UncapitalizedKebabCase } =>
        ...
        case '{ ConfigFieldStyle.CapitalizedSnakeCase } =>
          FieldStyle.capitalizedSnakecase
        case '{ ConfigFieldStyle.UncapitalizedSnakeCase } =>
          FieldStyle.uncapitalizedSnakecase
        ...
        case _ =>
          report.errorAndAbort(
            "fieldStyle in writer config can not be computed in compile-time"
          )
      }
    }
    ...
    val discriminator = discriminatorExpr.map(
      _.asTerm.underlying
        ...
        .getOrElse(
          report.errorAndAbort(
            "discriminator in writer config can not be computed in compile-time"
          )
        )
    )
