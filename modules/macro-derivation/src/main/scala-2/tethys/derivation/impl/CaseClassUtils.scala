...
trait CaseClassUtils extends LoggingUtils {
  ...
  def caseClassDefinition(tpe: Type): CaseClassDefinition = {
    ...
    CaseClassDefinition(
      tpe = tpe,
      fields = ctor.paramLists.head.zipWithIndex.map { case (sym, idx) =>
        constructorParameterToCaseClassField(tpe)(idx, sym)
      }
    )
  }
  ...
  private def constructorParameterToCaseClassField(
      tpe: Type
  )(idx: Int, param: Symbol): CaseClassField = {
    ...
    CaseClassField(
      ...
      defaultValue = if (param.asTerm.isParamWithDefault) {
