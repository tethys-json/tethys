...
trait ReaderDescriptionCommons extends ReaderBuilderUtils {
  ...
  def convertReaderBuilder[A: WeakTypeTag](
  ...
  private def extractDescription(tree: Tree): ReaderMacroDescription =
    tree match {
      ...
      case q"${rest: Tree}.extract[$tpe](${f: BuilderField}).from[..$_](..${fs: Seq[
              Field
            ]}).and[..$_](..${ands: Seq[Field]}).apply(${fun: Tree})" =>
        val description = extractDescription(rest)
        description.copy(operations =
          description.operations :+ ReaderMacroOperation.ExtractFieldValue(
            f.name,
      ...
      case q"${rest: Tree}.extractReader[$tpe](${f: BuilderField}).from[..$_](..${fs: Seq[
              Field
            ]}).and[..$_](..${ands: Seq[Field]}).apply(${fun: Tree})" =>
        val description = extractDescription(rest)
        description.copy(operations =
