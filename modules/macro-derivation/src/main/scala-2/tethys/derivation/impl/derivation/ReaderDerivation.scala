...
import tethys.derivation.impl.builder.{
  ReaderBuilderUtils,
...
trait ReaderDerivation
  ...
  private val primitiveReadersCompanion =
    q"$readersPack.instances.PrimitiveReaders"

  private val jsonReaderDefaultValueType =
    tq"$readersPack.JsonReaderDefaultValue"
  ...
  private case class SimpleField(
      name: String,
      tpe: Type,
      jsonName: String,
      value: TermName,
  ...
  private case class FromExtractedReader(
      ...
      value: TermName,
      isInitialized: TermName,
      tempIterator: TermName
  ) extends ReaderField

  private case class FunctionArgument(
      field: Field,
      value: TermName,
      isInitialized: TermName
  )
  ...
  def deriveReader[A: WeakTypeTag](
      description: ReaderMacroDescription
  ): Expr[JsonReader[A]] = {
    val tpe = weakTypeOf[A]
    val classDef = caseClassDefinition(tpe)
    val config = scala.util
      .Try(c.eval(description.config))
      .getOrElse(c.eval(description.config))
    ...
    val (typeDefaultValues, defaultValuesTrees) = allocateDefaultValues(
      readerFields
    )
        ...
        case Field.RawField(name, tpe) =>
          val possibleArg = readerFields.flatMap {
            ...
            case f: ExtractedField =>
              f.args.collectFirst {
                case arg @ FunctionArgument(rf: Field.RawField, _, _)
  ...
  private def allocateReaders(
      readerFields: List[ReaderField]
  ): (List[(Type, TermName)], List[Tree]) = {
    ...
    jsonTypes.foldLeft((List[(Type, TermName)](), List[Tree]())) {
      case ((types, trees), tpe) if !types.exists(_._1 =:= tpe) =>
        ...
        val reader = {
          if (tpe =:= typeOf[Short])
            q"private[this] val $term = $primitiveReadersCompanion.ShortJsonReader"
          else if (tpe =:= typeOf[Int])
            q"private[this] val $term = $primitiveReadersCompanion.IntJsonReader"
          else if (tpe =:= typeOf[Long])
            q"private[this] val $term = $primitiveReadersCompanion.LongJsonReader"
          else if (tpe =:= typeOf[Float])
            q"private[this] val $term = $primitiveReadersCompanion.FloatJsonReader"
          else if (tpe =:= typeOf[Double])
            q"private[this] val $term = $primitiveReadersCompanion.DoubleJsonReader"
          else if (tpe =:= typeOf[Boolean])
            q"private[this] val $term = $primitiveReadersCompanion.BooleanJsonReader"
          else
            q"private[this] lazy val $term = implicitly[$jsonReaderType[$tpe]]"
        }
  ...
  private def allocateDefaultValues(
      readerFields: List[ReaderField]
  ): (List[(Type, TermName)], List[Tree]) = {
  ...
  private def allocateVariables(
      readerFields: List[ReaderField],
      typeDefaultValues: List[(Type, TermName)]
  ): List[Tree] = {
    val possibleValues: List[(TermName, Type, Option[Tree])] =
      readerFields.flatMap {
        case f: SimpleField =>
          List((f.value, f.tpe, f.defaultValue))
        case f: ExtractedField =>
          (f.value, f.tpe, None) :: f.args.map(arg =>
            (arg.value, arg.field.tpe, None)
          )
        case f: FromExtractedReader =>
          ((f.value, f.tpe, None)) :: f.args.map(arg =>
            (arg.value, arg.field.tpe, None)
    ...
    val (_, values) = possibleValues.foldLeft(List[TermName](), List[Tree]()) {
      ...
      case ((allocated, trees), (value, tpe, defaultTreeOpt))
          if !allocated.contains(value) =>
        val tree =
          q"var $value: $tpe = ${typeDefaultValues.find(_._1 =:= tpe).get._2}"
        (value :: allocated, tree :: trees)
    ...
    val tempIterators = readerFields.collect { case f: FromExtractedReader =>
  ...
  private def allocateFunctions(readerFields: List[ReaderField]): List[Tree] =
    readerFields.collect {
      case f: ExtractedField =>
  ...
  private def allocateCases(
    ...
    case class SimpleFieldDef(
        jsonName: String,
        reader: TermName,
        value: TermName,
        isInitialized: TermName
    ) extends FieldDef
    case class CustomReaderFieldDef(jsonName: String, tempIterator: TermName)
        extends FieldDef
    ...
    val gropedDefs = fieldDefs.distinct
      .groupBy(_.jsonName)
      .toList
      .sortBy(f => fieldDefs.indexWhere(_.jsonName == f._1))
    ...
    val res = gropedDefs.map {
      ...
      case (jsonName, defs) =>
        ...
        val body =
          q"val $fieldIterator = $tokenIteratorTerm.collectExpression()" :: defs
            .flatMap {
              case SimpleFieldDef(_, reader, value, isInitialized) =>
  ...
  private def allocateRawFieldsPostProcessing(
    ...
    def buildTree(
        ...
        value: TermName,
        isInitialized: TermName
    ): Res => Res = { case (processed, possiblyNotInitialized, trees) =>
      extractDefaultValue(tpe) match {
        ...
        case None =>
          (
            value :: processed,
            (q"!$isInitialized", jsonName) :: possiblyNotInitialized,
            trees
          )
      }
    }

    val (_, possiblyNotInitialized, defaultValues) = readerFields.foldLeft(
      (List[TermName](), List[(Tree, String)](), List[Tree]())
    ) {
    ...
    possiblyNotInitialized.reverse match {
      ...
      case xs @ (headNotInit, _) :: tail =>
        ...
        val predicate =
          tail.foldLeft[Tree](q"$headNotInit")((a, b) => q"$a || ${b._1}")
        val fields = xs.map { case (notInit, name) =>
          q"""
               if($notInit) {
  ...
  private def allocateTransformationsLoop(
    ...
    val (_, possiblyNotInitializedFields, defaults, loopActions) =
      readerFields.foldLeft(
        (List[TermName](), List[(Tree, String)](), List[Tree](), List[Tree]())
      ) {
        case (
              (processed, possiblyNotInitialized, defaultTrees, loopTrees),
              field
            ) =>
          def buildTransformation(
              name: String,
              tpe: Type,
              args: List[FunctionArgument],
              value: TermName,
              isInitialized: TermName,
              tempIterator: Option[TermName]
          )(valueAction: Tree) = {
            ...
            val canProcess = args.foldLeft[Tree](start) {
              case (
                    current,
                    FunctionArgument(_: Field.ClassField, _, argIsInitialized)
                  ) =>
                q"$current && $argIsInitialized"
              case (
                    current,
                    _
                  ) => // RawField already checked in allocateRawFieldsPostProcessing
                current
            }
            ...
            extractDefaultValue(tpe) match {
              case Some(defaultValue) =>
                ...
                (
                  ...
                  tree :: defaultTrees,
                  loopAction :: loopTrees
                )

              case None =>
                (
                  value :: processed,
                  (q"!$isInitialized", name) :: possiblyNotInitialized,
                  defaultTrees,
                  loopAction :: loopTrees
                )
            }
  ...
  private def extractDefaultValue(tpe: Type): Option[Tree] = {
    c.typecheck(q"implicitly[$jsonReaderDefaultValueType[$tpe]]") match {
      case q"$_.implicitly[$_]($defaultValue)" =>
        val mbValue =
          defaultValue.tpe.typeSymbol.annotations.map(_.tree).collectFirst {
            case q"new $clazz(${value: Tree})"
                if clazz.tpe =:= typeOf[
        ...
        mbValue match {
          case None =>
            abort(
              s"JsonReaderDefaultValue '${defaultValue.tpe}' is not annotated with 'ReaderDefaultValue'"
            )
          case Some(q"null") =>
            None
          case Some(value) =>
