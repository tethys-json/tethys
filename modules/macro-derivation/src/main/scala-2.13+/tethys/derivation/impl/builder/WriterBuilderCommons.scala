...
trait WriterBuilderCommons extends WriteBuilderUtils {
  ...
  protected def convertWriterBuilder[A: WeakTypeTag](
      builder: Expr[WriterBuilder[A]]
  ): Expr[WriterDescription[A]] = {
  ...
  protected lazy val emptyWriterConfig: Expr[WriterDerivationConfig] =
    c.Expr[WriterDerivationConfig](
      c.untypecheck(
        q"tethys.derivation.builder.WriterDerivationConfig.empty"
      )
    )

  private def extractSimpleDescription(tree: Tree): MacroWriteDescription =
    tree match {
      // ===== ROOT =====
