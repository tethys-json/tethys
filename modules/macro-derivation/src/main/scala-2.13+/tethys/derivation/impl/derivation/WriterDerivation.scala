...
trait WriterDerivation
  ...
  def deriveWriterForSealedClass[A: WeakTypeTag](
    ...
    if (types.isEmpty) fail(s"${tpe.typeSymbol} has no known direct subclass")
      ...
      val writers = types.zip(terms).map { case (subtype, term) =>
        q"private[this] lazy val $term = implicitly[$jsonObjectWriterType[$subtype]]"
      }

      val subClassesCases = types.zip(terms).map { case (subtype, writer) =>
  ...
  def deriveWriter[A: WeakTypeTag](
    ...
    val fieldTrees = writerFields.map {
      ...
      case PartialExtractedField(_, jsonName, argExtractor, cases) =>
        ...
        val resultCases = cases.map { case CaseDef(d, g, body) =>
          val fieldTpe = unwrapType(body.tpe.finalResultType)
          val writerTerm = typeWriters.find(_._1 =:= fieldTpe).get._2
          CaseDef(
            d,
            g,
            q"$writerTerm.write(${jsonName.tree}, $body, $tokenWriterTerm)"
          )
        }
    ...
    fieldStyle.fold(writerFields) { style =>
      writerFields.map {
        case field: SimpleWriterField =>
          field.copy(jsonName =
            c.Expr[String](q"${style.applyStyle(field.name)}")
          )
        case field => field
      }
    }
        ...
        case BuilderMacroOperation.UpdatePartial(_, field, name, fun, from) =>
          mapField(fields, field)(f =>
            PartialExtractedField(
              ...
              cases = fun match {
                case q"{ case ..$cases }" =>
                  cases.asInstanceOf[Seq[CaseDef]].toList
              }
            )
          )
  ...
  private def allocateWriters(
    ...
    val types = writerFields.flatMap {
      ...
      case PartialExtractedField(_, _, _, cases) =>
        cases.map { case CaseDef(_, _, body) =>
          unwrapType(body.tpe.finalResultType)
        }
    }
      ...
      case ((terms, trees), tpe) if !terms.exists(_._1 =:= tpe) =>
        ...
        val tree = {
          if (tpe =:= typeOf[Nothing])
            q"private[this] lazy val $term = $writersPack.EmptyWriters.emptyWriter[Nothing]"
          else
            q"private[this] lazy val $term = implicitly[$jsonWriterType[$tpe]]"
        }
  ...
  private def allocateFunctions(writerFields: List[WriterField]): List[Tree] = {
    writerFields.flatMap {
      case SimpleWriterField(
            _,
            _,
            _,
            FunctionExtractor(name, _, from, to, body)
          ) =>
        q"private[this] val $name: $from => $to = $body" :: Nil
      case PartialExtractedField(
            _,
            _,
            FunctionExtractor(name, _, from, to, body),
            _
