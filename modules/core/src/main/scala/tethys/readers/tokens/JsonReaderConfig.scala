...
/** Configuration for [[com.github.plokhotnyuk.jsoniter_scala.core.JsonReader]]
  ...
  * runtime for parsing of messages. <br/> All configuration params already
  * initialized by recommended default values, but in some cases they should be
  * altered for performance reasons: <ul> <li>turn off stack traces for parsing
  * exceptions to greatly reduce impact on performance for cases when exceptions
  * can be not exceptional (e.g. under DoS attacks over open to the world
  * systems), see more details here:
  * [[https://shipilev.net/blog/2014/exceptional-performance/]]</li> <li>turn
  * off appending of hex dump to minimize length of exception message</li>
  * <li>increase preferred size of an internal byte buffer for parsing from
  * [[java.io.InputStream]] or [[java.nio.DirectByteBuffer]] to reduce
  * allocation rate of grown and then reduced buffers during parsing of large
  * (>16Kb) numbers (including stringified), raw values, or ADT instances with
  * the discriminator field doesn't appear in the beginning of the JSON
  * object</li> <li>increase preferred size of an internal char buffer to reduce
  * allocation rate of grown and then reduced buffers when large (>4Kb) string
  * instances need to be parsed including those one which use Base16 or Base64
  * encodings</li> </ul>
  * @param throwReaderExceptionWithStackTrace
  *   a flag that allows to turn on a stack traces for debugging purposes in
  *   development
  * @param appendHexDumpToParseException
  *   a flag that allows to turn off hex dumping of affected by error part of an
  ...
  *   values including those one which use Base16 or Base64 encodings
  ...
  * @param hexDumpSize
  *   a size of the hex dump in 16-byte lines before and after the 16-byte line
...
class ReaderConfig private (
  ...
  def withThrowReaderExceptionWithStackTrace(
      throwReaderExceptionWithStackTrace: Boolean
  ): ReaderConfig =
    copy(throwReaderExceptionWithStackTrace =
      throwReaderExceptionWithStackTrace
    )

  def withAppendHexDumpToParseException(
      appendHexDumpToParseException: Boolean
  ): ReaderConfig =
  ...
  def withMaxBufSize(maxBufSize: Int): ReaderConfig = {
    if (maxBufSize < preferredBufSize)
      throw new IllegalArgumentException(
        "'maxBufSize' should be not less than 'preferredBufSize'"
      )
    if (maxBufSize > 2147483645)
      throw new IllegalArgumentException(
        "'maxBufSize' should be not greater than 2147483645"
      )
  ...
  def withMaxCharBufSize(maxCharBufSize: Int): ReaderConfig = {
    if (maxCharBufSize < preferredCharBufSize)
      throw new IllegalArgumentException(
        "'maxCharBufSize' should be not less than 'preferredCharBufSize'"
      )
    if (maxCharBufSize > 2147483645)
      throw new IllegalArgumentException(
        "'maxCharBufSize' should be not greater than 2147483645"
      )
    copy(maxCharBufSize = maxCharBufSize)
  }

  def withPreferredBufSize(preferredBufSize: Int): ReaderConfig = {
    if (preferredBufSize < 12)
      throw new IllegalArgumentException(
        "'preferredBufSize' should be not less than 12"
      )
    if (preferredBufSize > maxBufSize)
      throw new IllegalArgumentException(
        "'preferredBufSize' should be not greater than 'maxBufSize'"
      )
  ...
  def withPreferredCharBufSize(preferredCharBufSize: Int): ReaderConfig = {
    if (preferredCharBufSize < 0)
      throw new IllegalArgumentException(
        "'preferredCharBufSize' should be not less than 0"
      )
    if (preferredCharBufSize > maxCharBufSize)
      throw new IllegalArgumentException(
        "'preferredCharBufSize' should be not greater than 'maxCharBufSize'"
      )
  ...
  def withHexDumpSize(hexDumpSize: Int): ReaderConfig = {
    if (hexDumpSize < 1)
      throw new IllegalArgumentException(
        "'hexDumpSize' should be not less than 1"
      )
