...
object TokenNode:
  ...
  def obj(fields: (String, Matchable)*): List[TokenNode] =
  ...
  def arr(elems: Matchable*): List[TokenNode] =
  ...
  private def anyToTokens(any: Matchable): List[TokenNode] = any match
    case v: TokenNode                => v :: Nil
    case nodes: List[? <: Matchable] => nodes.flatMap(anyToTokens)
  ...
  implicit class TokenNodesOps(val json: String) extends AnyVal:
    def jsonAsTokensList(implicit
      ...
      while !iterator.currentToken().isEmpty do
        val token = iterator.currentToken()
        val node =
          if token.isArrayStart then ArrayStartNode
          else if token.isArrayEnd then ArrayEndNode
          else if token.isObjectStart then ObjectStartNode
          else if token.isObjectEnd then ObjectEndNode
          else if token.isNullValue then NullValueNode
          else if token.isFieldName then FieldNameNode(iterator.fieldName())
          else if token.isStringValue then StringValueNode(iterator.string())
          else if token.isNumberValue then iterator.number() match
