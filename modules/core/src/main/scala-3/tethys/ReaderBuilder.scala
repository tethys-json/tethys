...
sealed trait ReaderBuilder[A]:
  def extract[Field](field: A => Field): ReaderBuilder.DependentFieldAs[A, Field]

  def extractReader[Field](field: A => Field): ReaderBuilder.DependentField0[A, JsonReader[_ <: Field]]
...
object ReaderBuilder:
  def apply[A](using mirror: scala.deriving.Mirror.ProductOf[A]): ReaderBuilder[A] =
  ...
  sealed trait DependentField1[A, Field, OneCollected]:
    ...
    (using ev: OneCollected *: EmptyTuple =:= mirror.MirroredElemTypes): ReaderBuilder[A]
