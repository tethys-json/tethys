...
private[tethys] object EnumCompanion:
...
private[derivation] object EnumCompanionMacro:
  ...
  def getByName[T: scala.quoted.Type](name: Expr[String])(using
    ...
    Select
      .unique(Ref(TypeRepr.of[T].typeSymbol.companionModule), "valueOf")
      .appliedToArgs(List(name.asTerm))
      .asExprOf[T]

  def getByOrdinal[T: scala.quoted.Type](ordinal: Expr[Int])(using
      quotes: Quotes
  ): Expr[T] =
    import quotes.reflect.*
    Select
      .unique(Ref(TypeRepr.of[T].typeSymbol.companionModule), "fromOrdinal")
