...
object StringEnumJsonReader:
  ...
  inline def derived[A <: scala.reflect.Enum](f: A => String): StringEnumJsonReader[A] =
    val valuesMap = derivation.EnumCompanion.getValues[A].map(x => f(x) -> x).toMap
  ...
  private def impl[A](get: String => A): TokenIterator => FieldName ?=> A = it =>
