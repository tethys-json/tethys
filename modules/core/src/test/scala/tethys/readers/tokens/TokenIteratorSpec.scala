...
class DefaultTokenIteratorSpec extends TokenIteratorSpec {
  override def producer: TokenIteratorProducer =
    TokenIteratorProducer.tethysTokenIteratorProducer
}
...
trait TokenIteratorSpec extends AnyFlatSpec with Matchers {
  ...
  def testCase(
    ...
    it should s"iterate over $json" in {
      val iter = json.toTokenIterator(using producer).fold(throw _, identity)
      assertions(iter)
    }
  }
